...
class RoomClient {
    ...
    async handleRoomInfo(room) {
        ...
        this.peers = new Map(JSON.parse(room.peers));
        // ##########################################
        // Нормализуем peer_cam_bubble из снимка комнаты (поздний join студента)
        try {
            this.peers.forEach((peer, peerId) => {
                if (peer?.peer_info) this.syncPeerCamBubbleState(peerId, peer.peer_info);
            });
        } catch {
            /* ignore */
        }
        ...
        for (let peer of Array.from(this.peers.keys()).filter((id) => id == this.peer_id)) {
            ...
            if (this._stageScene?.mode) {
                // Hide stray presenter cams immediately so guests never see 2 tiles before stage settles
                try {
                    this.bootstrapStageSceneHide();
                } catch {
                    /* ignore */
                }
                [100, 300, 800, 1600, 3200, 5000].forEach((ms) => setTimeout(() => this.applyStageScene(), ms));
            }
            try {
                this.installVideoGestureUnlock();
            } catch {
                /* ignore */
            }
            if (isPresenter) {
        ...
        // Кружок лектора после join/getProducers (тайлы появляются асинхронно)
        setTimeout(() => this.refreshAllCamBubbles(), 300);
        setTimeout(() => this.refreshAllCamBubbles(), 1200);
    ...
    handleNewProducers = async (data) => {
        if (data.length > 0) {
            ...
            for (let { producer_id, peer_name, peer_info, type } of data) {
                ...
                // Актуальный peer_info с сервера (в т.ч. peer_cam_bubble) — в map
                if (peer_info?.peer_id) this.syncPeerCamBubbleState(peer_info.peer_id, peer_info);
    ...
    handleUpdatePeerInfo = (data) => {
        ...
        if (data.type === 'camBubble') {
            this.syncPeerCamBubbleState(data.peer_id, {
                peer_cam_bubble: Boolean(data.status),
                peer_cam_bubble_layout: data.status ? undefined : null,
            });
            this.refreshCamBubble(data.peer_id);
            // Тайлы consumer'ов могут появиться чуть позже
            this.scheduleCamBubbleRetry(data.peer_id);
        }
        if (data.type === 'camBubbleLayout') {
            const encoded =
                typeof data.status === 'string'
                    ? data.status
                    : this.encodeCamBubbleLayout(this.decodeCamBubbleLayout(data.status));
            this.syncPeerCamBubbleState(data.peer_id, {
                peer_cam_bubble: true,
                peer_cam_bubble_layout: encoded,
            });
    ...
    async produce(type, deviceId = null, swapCamera = false, init = false) {
        ...
        try {
            if (init) {
                ...
                // Dialog invite: prefer soft constraints if strict ones fail
                if (!screen && this._forceSimpleMedia) {
                    try {
                ...
                if (video && isMediaStreamTrackAndTransformerSupported && !this._forceSimpleMedia) {
            ...
            if (audio && BUTTONS.settings.customNoiseSuppression && !this._forceSimpleMedia) {
                /*
        ...
        */
        if (deviceId) {
            if (this.isMobileSafari) {
                ...
                // Prefer ideal so stale deviceId does not hard-fail dialog invite
                audioConstraints.deviceId = { ideal: deviceId };
            }
        }
    ...
    getVideoConstraints(deviceId) {
        ...
        const customFrameRate = parseInt(selectedValue, 10) || 30;

        const resolutionMap = this.getResolutionMap();

        // Default to HD (null-safe: guests may not have settings UI ready)
        const qualityKey = videoQuality?.value || 'hd';
        const [width, height] = resolutionMap[qualityKey] || [1280, 720];

        const constraints = {
            width: { ideal: width },
            height: { ideal: height },
            frameRate: { ideal: customFrameRate },
        };
    ...
    async handleProducer(id, type, stream) {
        ...
        switch (type) {
            ...
            case mediaType.screen:
                ...
                elem.setAttribute('controlslist', 'nodownload nofullscreen noremoteplayback');
                elem.autoplay = true;
                elem.muted = true;
                elem.volume = 0;
    ...
    async handleConsumer(id, type, stream, peer_name, peer_info) {
        ...
        switch (type) {
            ...
            case mediaType.screen:
                ...
                // peer_info из newProducers — live с сервера, там уже есть peer_cam_bubble
                ...
                setTimeout(() => this.ensureDialogVideosPlaying?.(), 200);
                this.sound('joined');
                break;
            case mediaType.audio:
                elem = document.createElement('audio');
    ...
    async attachMediaStream(elem, stream, type, who) {
        ...
        if (type !== mediaType.audio) {
            ...
            // Tap must not open native player / pause — keep WebRTC video playing
            if (!elem._optrfNoNativeUi) {
                elem._optrfNoNativeUi = true;
                elem.addEventListener(
                    ...
                    (e) => {
                        ...
                        try {
                            elem.controls = false;
                            elem.removeAttribute('controls');
                            if (elem.paused) elem.play?.().catch?.(() => {});
                        } catch {
                            /* ignore */
                        }
                    },
    ...
    getSelectedIndexValue(elem) {
        try {
            if (!elem?.options || elem.selectedIndex < 0) return '';
    ...
    toggleFullScreen(elem = null) {
        // Не блокируем FS видео из‑за флага document FS — сверяем реальный fullscreenElement
        const getFsElement = () =>
        ...
        if (fsEl) {
            ...
            if (elem) this.isVideoOnFullScreen = false;
            return;
        }
        this.goInFullscreen(element);
        if (elem) this.isVideoOnFullScreen = true;
    }
    ...
    goInFullscreen(element) {
        ...
        if (req) {
            Promise.resolve(req.call(element)).catch((err) => {
                console.warn('requestFullscreen failed', err);
                this.userLog('warning', 'Не удалось открыть на весь экран', 'top-end', 4000);
            });
        } else {
            this.userLog('warning', 'Полноэкранный режим не поддерживается', 'top-end');
        }
    }
    ...
    handleFS(elemId, fsId) {
        ...
        if (btnFs.dataset.fsBound === '1') return;
        btnFs.dataset.fsBound = '1';
        ...
        const resolveFsTarget = () => {
            // Закреплённая трансляция — на весь экран весь pin-контейнер (с кружком)
            if (this.isVideoPinned && this.videoPinMediaContainer && videoWrap?.parentElement === this.videoPinMediaContainer) {
                return this.videoPinMediaContainer;
            }
            return videoWrap || videoPlayer;
        };
        ...
        const sync = () => {
            const fsEl = getFsElement();
            const target = resolveFsTarget();
            const isThisVideoFullscreen = fsEl === target || fsEl === videoWrap || fsEl === videoPlayer;
        ...
        btnFs.addEventListener('click', (e) => {
            ...
            this.toggleFullScreen(resolveFsTarget());
            setTimeout(sync, 0);
        });
    }
    ...
    makeDraggable(elmnt, dragObj) {
        ...
        function dragMouseDown(e) {
            e = e || window.event;
            e.preventDefault();
            // Снимаем translate(-50%) и т.п., иначе окно уезжает за край
            const rect = elmnt.getBoundingClientRect();
            elmnt.classList.remove('panel-slide-in');
            ...
            elmnt.style.top = `${Math.max(0, rect.top)}px`;
            pos3 = e.clientX;
            pos4 = e.clientY;
    ...
    makeUnDraggable(elmnt, dragObj) {
        ...
        // Не трогаем top/left — позицию задают chatPin / chatMaximize / chatCenter
    }
    ...
    async toggleChat(fromParticipants = false) {
        // Guests always may open public chat (broadcasting rules used to clear BUTTONS.main.chatButton)
        const guestMayChat = !isPresenter;
        if (!fromParticipants && !BUTTONS.main.chatButton && !guestMayChat) return;
        if (guestMayChat) BUTTONS.main.chatButton = true;
    ...
    async ensurePublicChatPinned() {
        try {
            ...
            console.warn('ensurePublicChatPinned failed', e);
        }
    }
    ...
    closeChatSidePanel() {
        ...
        if (this.isChatOpen) {
            ...
            this.toggleShowParticipants(true);
        }
        this.isParticipantsOpen = false;
        this.syncChatToolbarButtons();
    }
    ...
    toggleShowParticipants(fromUser = false) {
        ...
        // X на списке диалогов: всегда закрываем всю панель (не «общий чат»)
        if (isParticipantsListHidden && fromUser && this.isParticipantsOpen) {
    ...
    chatMaximize() {
        ...
        // На весь экран без translate(-50%) — иначе ломается из‑за panel-slide-in / pin right:0
        this.chatFillViewport();
        document.documentElement.style.setProperty('--msger-width', '100%');
        document.documentElement.style.setProperty('--msger-height', '100%');
    ...
    chatPin() {
        ...
        if (this._dialogSplitActive && !this._applyingDialogSplit) {
            this.applyDialogSplitLayout(this._dialogPresenterId, this._dialogGuestIds);
        } else if (this._stageScene?.mode && !this._applyingDialogSplit) {
            // Stage pin must shrink to 75% — chatPin previously skipped videoMediaContainerPin when pinned
            try {
                this.applyStageScene();
            } catch {
                /* ignore */
            }
        } else if (this.isVideoPinned && !this._applyingDialogSplit) {
            try {
    ...
    resetChatRoomPositionStyles() {
        chatRoom.classList.remove('panel-slide-in');
        chatRoom.style.position = 'fixed';
        chatRoom.style.inset = '';
        chatRoom.style.top = '';
        chatRoom.style.left = '';
        chatRoom.style.right = '';
    ...
    ensureChatReplyBar() {
        let bar = this.getId('chatReplyBar');
        if (bar) return bar;
        ...
        bar.innerHTML = `
            ...
            <div class="chat-reply-bar-body">
                ...
                <div class="chat-reply-bar-text" id="chatReplyBarText"></div>
            </div>
            <button type="button" id="chatReplyBarClose" class="chat-reply-bar-close" title="Отмена">
                <i class="fas fa-times"></i>
            </button>
        `;
        wrapper.insertBefore(bar, wrapper.firstChild);
        bar.querySelector('#chatReplyBarClose')?.addEventListener('click', (e) => {
            e.preventDefault();
            e.stopPropagation();
            this.clearChatReply();
        });
    ...
    setChatReply(msgListId) {
        const msgEl = document.getElementById(msgListId);
        if (!msgEl) return;
        const fromName = msgEl.getAttribute('data-from-name') || 'Участник';
        const msgId = msgEl.getAttribute('data-msg-id') || msgListId;
        const textEl = msgEl.querySelector('[id^="message-"]');
        const text = (textEl?.innerText || textEl?.textContent || '').trim().slice(0, 280);
        this._pendingChatReply = { msgId, fromName, text };
        const bar = this.ensureChatReplyBar();
        if (!bar) return;
        const nameEl = this.getId('chatReplyBarName');
        const textPreview = this.getId('chatReplyBarText');
        if (nameEl) nameEl.textContent = fromName;
        if (textPreview) textPreview.textContent = text || 'Сообщение';
        bar.classList.remove('hidden');
        chatMessage?.focus?.();
    }

    clearChatReply() {
        this._pendingChatReply = null;
        const bar = this.getId('chatReplyBar');
    ...
    sendMessage() {
        ...
        const reply = this._pendingChatReply
            ? {
                  reply_to_msg_id: this._pendingChatReply.msgId || '',
                  reply_to_peer_name: this._pendingChatReply.fromName || '',
                  reply_to_text: this._pendingChatReply.text || '',
              }
            : null;
        const data = {
            room_id: this.room_id,
    ...
    async showMessage(data, toggleChat = true) {
        ...
        const reply =
            data.reply_to_msg_id || data.reply_to_peer_name || data.reply_to_text
                ? {
                      reply_to_msg_id: data.reply_to_msg_id || '',
                      reply_to_peer_name: data.reply_to_peer_name || '',
                      reply_to_text: data.reply_to_text || '',
    ...
    appendMessage(side, img, fromName, fromId, msg, toId, toName, msgId = '', reply = null) {
        ...
        const replyHtml =
            replyName || replyText
                ? `<div class="chat-msg-reply" title="Ответ на сообщение">
                        <div class="chat-msg-reply-name">${this.sanitizeHtml(replyName || 'Участник')}</div>
                        <div class="chat-msg-reply-text">${this.sanitizeHtml(replyText || 'Сообщение')}</div>
                   </div>`
                : '';
        ...
        if (!this.isMobileDevice) {
            this.setTippy('msg-reply-' + chatMessagesId, 'Ответить', 'top');
    ...
    openVideo(data) {
        ...
        fsBtn.addEventListener('click', () => {
            ...
            if (
                ...
                this.isFullScreen() ? this.goOutFullscreen() : this.goInFullscreen(video);
            } else {
    ...
    exitDialogSplitLayout() {
        ...
        this.videoPinMediaContainer.style.top = '';
        this.videoPinMediaContainer.style.left = '';
    ...
    /**
     * Before screen/cam tiles arrive: hide every Camera except stage targets
     * so late-joining guests never see separate moderator + creator cams.
     */
    bootstrapStageSceneHide() {
        if (!this._stageScene?.mode) return;
        document.body.classList.add('stage-scene-active');
        document.body.classList.toggle('stage-scene-1', this._stageScene.mode === 1);
        document.body.classList.toggle('stage-scene-2', this._stageScene.mode === 2);
        const creatorId = this._stageScene.creatorId || this._meetingCreatorId;
        const modId = this._stageScene.moderatorId || this.findScreenSharingModeratorId();
        const keepIds = new Set(
            [creatorId, ...(this._dialogSplitActive ? this._dialogGuestIds || [] : [])].filter(Boolean)
        );
        const screenTile = modId ? this.getPeerMediaTile(modId, 'screen') : null;
        const creatorTile =
        ...
        document.querySelectorAll('#videoMediaContainer .Camera, #videoPinMediaContainer .Camera').forEach((el) => {
            const peerId =
                el.querySelector('video[name]')?.getAttribute('name') ||
                el.dataset?.peerId ||
                (el.id || '').replace(/__(videoOff|video|dialogSlot)$/, '');
            const kind = el.dataset?.mediaKind || '';
            const isKeepScreen = this._stageScene.mode === 1 && screenTile && el === screenTile;
            const isKeepCreator = creatorTile && el === creatorTile && this._stageScene.mode === 2;
            const isDialogGuest = peerId && keepIds.has(peerId) && kind !== 'screen' && peerId !== modId;
            if (isKeepScreen || isKeepCreator || isDialogGuest) {
                el.dataset.stageHidden = '0';
                el.style.removeProperty('display');
                return;
            }
            // Mode 1: hide moderator webcam / videoOff (screen is the stage)
            if (this._stageScene.mode === 1 && peerId === modId && kind !== 'screen') {
                el.dataset.stageHidden = '1';
                el.style.setProperty('display', 'none', 'important');
                return;
            }
            if (peerId && keepIds.has(peerId) && this._stageScene.mode === 1 && el === creatorTile) {
                // Creator cam feeds bubble — hide as grid tile
                el.classList.add('cam-bubble-source-hidden');
                el.dataset.stageHidden = '1';
                el.style.setProperty('display', 'none', 'important');
                return;
            }
    ...
    /** Force-play dialog / stage videos and resume their consumers (fixes iOS play overlay + AQ pause). */
    ensureDialogVideosPlaying() {
        try {
            const peerIds = new Set();
            if (this._dialogPresenterId) peerIds.add(this._dialogPresenterId);
            (this._dialogGuestIds || []).forEach((id) => peerIds.add(id));
            if (this._stageScene?.creatorId) peerIds.add(this._stageScene.creatorId);
            if (this._stageScene?.moderatorId) peerIds.add(this._stageScene.moderatorId);

            document.querySelectorAll('#videoMediaContainer video, #videoPinMediaContainer video, .cam-bubble video').forEach((v) => {
                try {
                    v.controls = false;
            ...
            // Resume AQ-paused consumers for dialog/stage peers
            this.consumers?.forEach?.((consumer, consumerId) => {
                if (consumer?.kind !== 'video') return;
                const tile = this.getId?.(consumerId + '__video');
                const peerId = tile?.dataset?.peerId || tile?.querySelector?.('video[name]')?.getAttribute('name');
                if (!peerId || !peerIds.has(peerId)) return;
                if (tile?.dataset?.stageHidden === '1' && !tile?.classList?.contains('cam-bubble-source-hidden')) {
                    return;
                }
                try {
                    this._aq?._pauseCache?.set?.(consumerId, false);
                    if (consumer.paused) {
                        this.socket
                            ?.request?.('resumeConsumer', { consumer_id: consumerId, type: 'video' })
                            ?.catch?.(() => {});
                        consumer.resume?.();
                    }
                } catch {
    ...
    installVideoGestureUnlock() {
        ...
        this._videoGestureUnlockInstalled = true;
        let last = 0;
        const unlock = () => {
            const now = Date.now();
    ...
    applyStageScene() {
        ...
        if (mode === 2) {
            ...
            if (aliveGuests.length) {
                ...
                // Solo scene 2: only creator camera full-bleed — leave room for pinned chat
                ...
                if (this.videoPinMediaContainer) {
                    ...
                    this.videoPinMediaContainer.style.setProperty('width', `${stagePct}%`, 'important');
        ...
        const screenTile = this.getPeerMediaTile(modId, 'screen');
        if (!screenTile) {
            // Keep a single stage surface while waiting for screen — never show 2 cams
            this.bootstrapStageSceneHide();
            const creatorFallback =
                this.getPeerMediaTile(creatorId, 'video') || this.getId?.(creatorId + '__videoOff');
            if (creatorFallback) {
                this.forcePinMediaTile(creatorFallback);
                this.hideNonStageTiles([creatorFallback, ...guestKeep].filter(Boolean), dialogGuests);
            } else {
                this.hideNonStageTiles(guestKeep, dialogGuests);
            }
            layoutStage();
            this.ensureDialogVideosPlaying?.();
    ...
    async peerMediaStartForced(type) {
        ...
        try {
            switch (type) {
                case mediaType.audio:
                    if (this.producerExist(mediaType.audio)) {
                        ...
                        try {
                            // Prefer null deviceId — avoid stale exact device failures for guests
                            await this.produce(mediaType.audio, null);
                        } catch (e1) {
                            console.warn('Audio produce fallback', e1);
                            await this.produceSimpleTrack(mediaType.audio);
                        }
                        if (!this.producerExist(mediaType.audio)) {
                            await this.produceSimpleTrack(mediaType.audio);
                        }
                    ...
                    if (!this.producerExist(mediaType.audio)) {
                        throw new Error('Audio producer not started');
                    }
                ...
                case mediaType.video:
                    if (this.producerExist(mediaType.video)) {
                        ...
                        try {
                            ...
                            console.warn('Video produce fallback', e1);
                            await this.produceSimpleTrack(mediaType.video);
                        }
                        if (!this.producerExist(mediaType.video)) {
                            await this.produceSimpleTrack(mediaType.video);
                        }
                    ...
                    if (!this.producerExist(mediaType.video)) {
                        throw new Error('Video producer not started');
                    }
            ...
            this._forceSimpleMedia = false;
        }
    }

    /** Retry produce with simplest constraints for dialog invite. */
    async produceSimpleTrack(type) {
        if (this.producerLabel.has(type)) {
            await this.resumeProducer(type);
            return;
        }
        this._forceSimpleMedia = true;
        await this.produce(type, null, false, false);
    }
    ...
    syncPeerCamBubbleState(peerId, info) {
        if (!peerId || !info) return;
        let peer = this.peers?.get?.(peerId);
        if (!peer) {
            peer = { peer_info: { peer_id: peerId } };
            this.peers.set(peerId, peer);
        }
        if (!peer.peer_info) peer.peer_info = { peer_id: peerId };
        if (info.peer_cam_bubble !== undefined) {
            peer.peer_info.peer_cam_bubble = Boolean(info.peer_cam_bubble);
            if (!info.peer_cam_bubble) peer.peer_info.peer_cam_bubble_layout = null;
        }
        if (info.peer_cam_bubble_layout !== undefined && info.peer_cam_bubble_layout !== null) {
            peer.peer_info.peer_cam_bubble_layout = info.peer_cam_bubble_layout;
    ...
    getPeerMediaTile(peerId, mediaKind) {
        ...
        // fallback: камера по name=peerId (локальная и remote)
        if (mediaKind === 'video') {
            const video = document.querySelector(`video[name="${peerId}"]`);
            const cam = video?.closest('.Camera, .has-cam-bubble, div');
            if (cam) {
                if (!cam.dataset.peerId) cam.dataset.peerId = peerId;
                if (!cam.dataset.mediaKind) cam.dataset.mediaKind = 'video';
                return cam;
            }
        }
        return null;
    ...
    scheduleCamBubbleRetry(peerId, attempts = 8) {
        if (!peerId) return;
        if (!this._camBubbleRetryTimers) this._camBubbleRetryTimers = new Map();
        const prev = this._camBubbleRetryTimers.get(peerId);
        ...
        const tick = () => {
            ...
            this._camBubbleRetryTimers.set(peerId, t);
        };
        const t = setTimeout(tick, 80);
        this._camBubbleRetryTimers.set(peerId, t);
    }

    refreshAllCamBubbles() {
        try {
            if (this.peer_info?.peer_cam_bubble) this.scheduleCamBubbleRetry(this.peer_id);
            this.peers?.forEach?.((peer, peerId) => {
                if (peerId === this.peer_id) return;
                if (peer?.peer_info?.peer_cam_bubble) this.scheduleCamBubbleRetry(peerId);
            });
        } catch (err) {
            console.warn('refreshAllCamBubbles', err);
        }
    }
    ...
    decodeCamBubbleLayout(raw) {
        ...
        // legacy: x,y,size,scale,ox,oy,border
        ...
        return this.normalizeCamBubbleLayout({
            ...
            ox: p[4],
            oy: p[5],
            border: p[6] === undefined ? true : Number(p[6]) !== 0,
        });
    }
    ...
    applyRemoteCamBubbleLayout(peerId, layoutRaw) {
        if (!peerId) return;
        const layout = this.decodeCamBubbleLayout(layoutRaw);
        const screenTile = this.getPeerMediaTile(peerId, 'screen');
        ...
        this.refreshCamBubble(peerId);
        this.scheduleCamBubbleRetry(peerId);
        return false;
    }
    ...
    ensureCamBubbleControlPanel() {
        ...
        panel.innerHTML = `
            <div class="cam-bubble-panel-head" title="Перетащите панель">
                <span class="cam-bubble-panel-drag"><i class="fas fa-grip-vertical"></i> Кружок камеры</span>
            ...
            <label class="cam-bubble-panel-slider">
                <span>Размер</span>
                <input type="range" id="camBubbleSizeRange" min="12" max="42" step="1" value="20" />
            </label>
            <label class="cam-bubble-panel-slider">
                <span>Масштаб</span>
        ...
        if (!this._camBubbleOutsideCloseBound) {
            ...
            // Только иконка у кружка — панель независима
            window.addEventListener('resize', () => this.positionCamBubbleEditBtn());
            window.addEventListener('scroll', () => this.positionCamBubbleEditBtn(), true);
        }

        return panel;
    }

    bindCamBubblePanelDrag(panel) {
        if (panel.dataset.dragBound === '1') return;
        panel.dataset.dragBound = '1';
        const head = panel.querySelector('.cam-bubble-panel-head');
        if (!head) return;
        ...
        head.addEventListener('pointerdown', (e) => {
            ...
            if (e.target.closest('.cam-bubble-panel-close')) return;
            e.preventDefault();
            e.stopPropagation();
    ...
    centerCamBubblePanel() {
        const panel = this.ensureCamBubbleControlPanel();
        panel.classList.add('show');
        // После display:flex нужен кадр для размеров
        const pw = panel.offsetWidth || 240;
        const ph = panel.offsetHeight || 200;
        const left = Math.max(8, Math.round((window.innerWidth - pw) / 2));
        const top = Math.max(8, Math.round((window.innerHeight - ph) / 2));
        panel.style.left = `${left}px`;
        panel.style.top = `${top}px`;
        panel.style.right = 'auto';
        panel.style.bottom = 'auto';
    ...
    /** Только иконка карандаша у кружка — панель к кружку не привязана. */
    positionCamBubbleEditBtn() {
    ...
    setCamBubblePanelOpen(open) {
        ...
        const bubble = this._camBubbleEl;
        if (bubble) {
            bubble.classList.toggle('panel-open', this._camBubblePanelOpen);
            if (!this._camBubblePanelOpen) {
                // Закрыли панель — выходим из «Кадр», нельзя двигать лицо/зум
        ...
        if (this._camBubblePanelOpen) {
            if (bubble?.dataset?.layout) {
                this.syncCamBubbleControlPanel(this.decodeCamBubbleLayout(bubble.dataset.layout));
            }
            this.centerCamBubblePanel();
        }
        this.positionCamBubbleEditBtn();
    }
    ...
    applyCamBubble(screenPeerId, camPeerId = null) {
        ...
        if (editable) {
            ...
            this.bindCamBubbleEditor(bubble, bubbleVideo, screenTile);
            this._camBubbleEl = bubble;
            bubble.classList.toggle('panel-open', Boolean(this._camBubblePanelOpen));
            if (!this._camBubblePanelOpen) {
                bubble.dataset.editMode = 'move';
                this._camBubbleEditMode = 'move';
    ...
    bindCamBubbleEditor(bubble, bubbleVideo, screenTile) {
        if (bubble.dataset.editorBound === '1') {
            // Обновляем API для панели даже если слушатели уже висят
            this._camBubbleEditor = this._makeCamBubbleEditorApi(bubble, bubbleVideo, screenTile);
            return;
        }
        ...
        bubble.addEventListener('pointerdown', (e) => {
            ...
            // Редактирование только при открытой панели
            if (!this._camBubblePanelOpen) return;
            e.preventDefault();
            e.stopPropagation();
            const isResize = e.target.classList?.contains('cam-bubble-resize');
            const editMode = bubble.dataset.editMode || 'move';
            // Кадр (pan) — только в режиме «Кадр»; иначе двигаем/ресайзим кружок
            if (isResize) dragMode = 'resize';
            else if (editMode === 'crop') dragMode = 'crop';
            else dragMode = 'move';
            startX = e.clientX;
            startY = e.clientY;
        ...
        bubble.addEventListener('pointermove', (e) => {
            if (!this._camBubblePanelOpen || !dragMode || !startLayout) return;
        ...
        bubble.addEventListener(
            'wheel',
            (e) => {
                // Зум лица — только панель открыта + режим «Кадр»
                if (!this._camBubblePanelOpen) return;
                if ((bubble.dataset.editMode || 'move') !== 'crop') return;
                e.preventDefault();
                e.stopPropagation();
                const cur = readLayout();
    ...
    _makeCamBubbleEditorApi(bubble, bubbleVideo, screenTile) {
        const readLayout = () => this.decodeCamBubbleLayout(bubble.dataset.layout || '');
        const commit = (layout, broadcast, immediate = true) => {
            const L = this.applyCamBubbleLayoutStyles(bubble, bubbleVideo, layout, true);
            const encoded = this.encodeCamBubbleLayout(L);
    ...
    broadcastCamBubbleLayout(layout, immediate = false) {
        const send = () => {
            const L = this.normalizeCamBubbleLayout(layout);
            const encoded = this.encodeCamBubbleLayout(L);
            this.peer_info.peer_cam_bubble_layout = encoded;
            this.updatePeerInfo(this.peer_name, this.peer_id, 'camBubbleLayout', encoded, true);
        };
        ...
        // Троттл на drag; финальный commit всегда шлёт immediate=true
        this._camBubbleLayoutTimer = setTimeout(send, 40);
    }
    ...
    async setCamBubbleEnabled(enabled) {
        ...
        if (on) {
            this.broadcastCamBubbleLayout(this.peer_info.peer_cam_bubble_layout, true);
            // Повторная рассылка — на случай, если студент ещё не успел создать тайлы
            setTimeout(() => {
                this.updatePeerInfo(this.peer_name, this.peer_id, 'camBubble', true, true);
                this.broadcastCamBubbleLayout(this.peer_info.peer_cam_bubble_layout, true);
            }, 400);
        }
